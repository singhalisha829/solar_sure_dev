...
const ContingencyItems = dynamic(
  () =>
    import(
      "@/components/project-components/ProjectDetails/ProjectContingency/ContingencyItemsForm"
    )
);
  ...
  const fetchContigencyDetails = async (id, installationItemList) => {
    await requestHandler(
      ...
      (data) => {
        ...
        const status = data.data.output[0].status;
        ...
        data.data.output[0].item_list.map((item) => {
          ...
          if (
            ...
            if (item.is_replaced) {
              ...
              // if the contingency status is incomplete, reset the replaced items table data
              if (status === "Incomplete") {
                replaced_other_item_list.push({
                  ...data,
                  replacement_total_amount: 0,
              ...
              if (status === "Incomplete") {
                replaced_item_list.push({
                  ...
                  replacement_quantity:
                    selectedBomItem[0]?.quantity?.quantity || 0,
                  replacement_total_amount: 0,
                  quantity: selectedBomItem[0]?.quantity?.quantity || 0,
                  total_amount: selectedBomItem[0]?.bbu_total_price,
                });
              } else {
                replaced_item_list.push({
                  ...data,
        ...
        setContingencyDetails({ id: id, remark: data.data.output[0].remark });
        ...
        {currentStep == 0 && (
          <ContingencyForm
            ...
            onSuccessfullSubmit={(id, remark) => {
              setCurrentStep(1);
              setContingencyDetails({ id: id, remark: remark });
            }}
          />
        )}
