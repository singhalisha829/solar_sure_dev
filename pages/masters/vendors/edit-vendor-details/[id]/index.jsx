...
import { getStates, getCity, getVendorDetails, editVendorDetails } from "@/services/api";
...
import { checkPincodeValue } from "@/utils/formValidationHandler";

const EditVendorRegistrationForm = () => {
    ...
    const fetchVendorDetails = async (id) => {
        await requestHandler(
            async () => await getVendorDetails(id),
            null,
            async (data) => {
                const vendorDetails = data.data.output.vendors_data[0];
                const correspondenceAddress = vendorDetails && vendorDetails?.addresses?.length > 0 ? vendorDetails?.addresses.find((addr) => addr.address_type === "correspondence") : '';
                const bankAddress = vendorDetails && vendorDetails?.addresses?.length > 0 ? vendorDetails?.addresses.find((addr) => addr.address_type === "bank") : '';
                const coiDocument = vendorDetails && vendorDetails?.vendor?.documents?.length > 0 ? vendorDetails.vendor.documents.find((doc) => doc.document_name === "COI Document") : null;
                const aoaDocument = vendorDetails && vendorDetails?.vendor?.documents?.length > 0 ? vendorDetails.vendor.documents.find((doc) => doc.document_name === "AOA Document") : null;
                const moaDocument = vendorDetails && vendorDetails?.vendor?.documents?.length > 0 ? vendorDetails.vendor.documents.find((doc) => doc.document_name === "MOA Document") : null;
                const panDocument = vendorDetails && vendorDetails?.vendor?.documents?.length > 0 ? vendorDetails.vendor.documents.find((doc) => doc.document_name === "PAN Document") : null;
                const gstDocument = vendorDetails && vendorDetails?.vendor?.documents?.length > 0 ? vendorDetails.vendor.documents.find((doc) => doc.document_name === "GST Certificate") : null;
                const cancelledCheque = vendorDetails && vendorDetails?.vendor?.documents?.length > 0 ? vendorDetails.vendor.documents.find((doc) => doc.document_name === "Cancelled Cheque") : null;
                if (vendorDetails.vendor.state) {
                    await getCityHandler(vendorDetails.vendor.state, 'state');
                }
                if (correspondenceAddress?.state) {
                    await getCityHandler(correspondenceAddress?.state, 'corr_state');
                }
                if (bankAddress?.state) {
                    await getCityHandler(bankAddress?.state, 'branch_state');
                }

                setFormData({
                    company_name: vendorDetails.vendor.name,
                    cin_number: vendorDetails.vendor.cin_no,
                    pan_number: vendorDetails.vendor.pan,
                    gst_number: vendorDetails.vendor.gst,
                    poc_name: vendorDetails.vendor.contact_person_name,
                    poc_phone: vendorDetails.vendor.contact_no,
                    poc_telephone: vendorDetails.vendor.telephone_no,
                    poc_fax: vendorDetails.vendor.fax_no,
                    poc_email: vendorDetails.vendor.email,
                    address: vendorDetails.vendor.address,
                    city: vendorDetails.vendor.city,
                    state: vendorDetails.vendor.state,
                    pincode: vendorDetails.vendor.pincode,
                    corr_address_id: correspondenceAddress?.id || "",
                    corr_address: correspondenceAddress?.address,
                    corr_city: correspondenceAddress?.city,
                    corr_state: correspondenceAddress?.state,
                    corr_pincode: correspondenceAddress?.pincode,
                    account_type: vendorDetails.bank_accounts[0]?.account_type || "",
                    bank_details_id: vendorDetails.bank_accounts[0]?.id || "",
                    bank_name: vendorDetails.bank_accounts[0]?.bank_name || "",
                    branch_name: vendorDetails.bank_accounts[0]?.branch_name || "",
                    branch_address_id: bankAddress?.id || "",
                    branch_address: bankAddress?.address || "",
                    branch_city: bankAddress?.city || "",
                    branch_state: bankAddress?.state || "",
                    branch_pincode: bankAddress?.pincode || "",
                    account_holder_name: vendorDetails.bank_accounts[0]?.account_holder_name || "",
                    account_number: vendorDetails.bank_accounts[0]?.account_number || "",
                    ifsc_code: vendorDetails.bank_accounts[0]?.ifsc_code || "",
                    rm_name: vendorDetails.bank_accounts[0]?.name_of_rm || "",
                    rm_phone: vendorDetails.bank_accounts[0]?.rm_contact_no || "",
                    branch_code: vendorDetails.bank_accounts[0]?.branch_code || "",
                    currency_type: vendorDetails.bank_accounts[0]?.currency || "",
                    customer_id_with_banker: vendorDetails.bank_accounts[0]?.customer_id_with_bank || "",
                    crn_no: vendorDetails.bank_accounts[0]?.crn_number || "",
                    swift_code: vendorDetails.bank_accounts[0]?.swift_code || "",
                    micr_code: vendorDetails.bank_accounts[0]?.micr_code || "",
                    account_holder_contact: vendorDetails.bank_accounts[0]?.mobile_no_updated_in_bank || "",
                    account_holder_telephone: vendorDetails.bank_accounts[0]?.telephone_no_updated_in_bank || "",
                    account_type_limit: vendorDetails.bank_accounts[0]?.cc_od_limit || "",
                    documents: {
                        "COI Document": { document_name: coiDocument?.document_name || "", url: coiDocument?.document || "", document: coiDocument?.document_url || "" },
                        "MOA Document": { document_name: moaDocument?.document_name || "", url: moaDocument?.document || "", document: moaDocument?.document_url || "" },
                        "AOA Document": { document_name: aoaDocument?.document_name || "", url: aoaDocument?.document || "", document: aoaDocument?.document_url || "" },
                        "PAN Document": { document_name: panDocument?.document_name || "", url: panDocument?.document || "", document: panDocument?.document_url || "" },
                        "GST Certificate": { document_name: gstDocument?.document_name || "", url: gstDocument?.document || "", document: gstDocument?.document_url || "" },
                        "Cancelled Cheque": { document_name: cancelledCheque?.document_name || "", url: cancelledCheque?.document || "", document: cancelledCheque?.document_url || "" },
                    },
                });
                setIsLoading(false);
            },
    ...
    const handleSubmitForm = async () => {
        const payload = buildVendorPayload({
            ...
            bank_details_id: formData.bank_details_id,
            corr_address_id: formData.corr_address_id,
            branch_address_id: formData.branch_address_id
        });

        await requestHandler(
            async () => await editVendorDetails(id, payload),
            null,
            async (data) => {
...
export default EditVendorRegistrationForm;