...
import {
  addProjectPaymentInvoice,
  editProjectPaymentInvoice,
} from "@/services/api";
...
const AddSgPaymentInvoiceModal = ({
  modalId,
  projectId,
  ...
  const onSubmit = async () => {
    ...
    const postData = {
      ...paymentDetails,
      invoice_category: "SG Ornate Solar Pvt Ltd",
    };
    ...
    if (modalId.split("-")[0] === "add") {
      ...
      await requestHandler(
        async () =>
          await editProjectPaymentInvoice(paymentDetails.id, editData),
        null,
        async (data) => {
          closeModal(modalId);
  ...
  return (
    <FormModal
      ...
      heading={
        modalId.split("-")[0] === "add"
          ? "Add SG Ornate Invoice"
          : "Edit SG Ornate Invoice"
      }
      onClose={() => {
        if (modalId.split("-")[0] === "add") {
          clearForm();
...
export default AddSgPaymentInvoiceModal;
