...
const getRowBackgroundColor = (row, index, { hightlightClosedProjects, highlightPoItems, hightlightCancelledProject, hightlightHoldProject, hightlightActiveProject }) => {
  if (hightlightCancelledProject && row.status === "Cancelled") {
    return "bg-slate-200 line-through";
  }
  if (hightlightHoldProject && row.status === "Hold") {
    return "bg-orange-100";
  }
  if ((hightlightClosedProjects && row.status === "Closed") || (highlightPoItems && row?.quantity_left_after_purchase_order?.quantity === 0)) {
    return "bg-zinc-100";
  }
  if (hightlightActiveProject && row.status === "Active") {
    return "bg-white";
  }
  return index % 2 !== 0 ? "bg-primary-light-5" : "bg-white";
};
...
const Table = ({
  ...
  const requestSort = (key) => {
    ...
    // if same column, toggle through 3 states
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`flex items-center gap-1.5 transition-colors duration-100 ${column.sortable ? "cursor-pointer hover:text-primary" : ""}`}>
                    {column.name}
                    {column.sortable && (
                      <span className="ml-0.5 text-primary">
                        {sortConfig.key !== column.key && <LuArrowDownUp />}
                        {sortConfig.key === column.key && sortConfig.direction === null && <LuArrowDownUp />}
                        {sortConfig.key === column.key && sortConfig.direction === "ascending" && <LuMoveUp />}
                        {sortConfig.key === column.key && sortConfig.direction === "descending" && <LuMoveDown />}
                      </span>
                    )}
                  </span>
                </div>
      ...
      <tbody>
        ...
        {sortedRows.map((row, index) => (
          <tr
            ...
            className={`${onRowClick ? "cursor-pointer" : ""} ${getRowBackgroundColor(row, index, { hightlightClosedProjects, highlightPoItems: restProps?.highlightPoItems, hightlightCancelledProject: restProps?.hightlightCancelledProject, hightlightHoldProject: restProps?.hightlightHoldProject, hightlightActiveProject: restProps?.hightlightActiveProject })}`}
          >
            {restProps.showCheckbox && (
              <td
                className={`px-4 py-3 text-xs w-[4rem] max-w-[4rem] min-w-[4rem] font-semibold uppercase sticky left-0 bg-inherit `}
            ...
            {columns.map((column, innerIndex) => {
              ...
              if (column.key === "status") {
              ...
              } else if (column.key === "is_additional_po_added") {
                display_content = (
                  <div className="flex flex-col justify-center gap-2.5 text-zinc-600 text-xs font-semibold capitalize">
                ...
                display_content = (
                  <span className="flex gap-4 justify-center">
                    ...
                    {column?.actionType?.split("-").includes("mdl_revision")
                      && row?.document
                      && (
                        <FaRedoAlt
                          title="Revise"
              ...
              } else if (column.type === "edit-contingency-actions-column") {
                display_content = (
                  <span className="flex gap-4 justify-center">
                    {row.contingency_type != "quantity" && (
                      <FaPen
                        title="Edit"
                        className="text-stone-300 text-[13px] cursor-pointer hover:text-zinc-600"
                        onClick={(e) => {
